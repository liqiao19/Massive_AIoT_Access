function [H_set, Location_Central_suba, Location_UEs, D] = New_Channel_Model(FacSp, AUD_Index_K_set, M_suba, N_sub, fs, fc, Delta_suba, Subc_Index_set, N_subc, K, Ka, Lp_max, sigma_2_alpha)
...
for kk_2 = 1:Ka
    Lp_k = randi(Lp_max);
%     Lp_k = 1;
    alpha_k_set = sort(sqrt(sigma_2_alpha/2)*(randn(Lp_k,1)+1i*randn(Lp_k,1)),'descend');   %小尺度衰落
...
%         save channel3.mat Subc_Index_set fs delta_subc N_subc tau_klm D
    else
...
%     save channel3.mat H_k_set h D Sel Lp_k gamma_k
    ...
    % subplot(2,3,kk_2)
    % plot(1:size(H_k_set,1), abs(sum(H_k_set, 2)))
    % title(['UE-ID', num2str(AUD_Index_K_set(kk_2))])
end
