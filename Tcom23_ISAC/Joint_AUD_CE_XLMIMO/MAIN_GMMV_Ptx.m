...
for nn_s = 1:Num_sim
    ...
    for gg_1 = 1:G_ofdm
        ...
        % Rx_Sig_mmv =reshape(pagemtimes(permute(H_set(:,1:P_subc,AUD_Index_K_set(1:Ka))*sqrt(Ptx),[1,3,2]),reshape(s_pilot_2(AUD_Index_K_set(1:Ka),1:P_subc),[Ka,1,P_subc])), M_suba*N_sub, P_subc);
        % Y_ALL_set((gg_1-1)*M_suba+1:gg_1*M_suba,:,iii) = W_g_set(:,:,1)'*Rx_Sig_mmv;
        % Z_ALL_set((gg_1-1)*M_suba+1:gg_1*M_suba,:,iii) = kron(sqrt(Ptx)*s_pilot_2(:,1).',W_g_set(:,:,1)');
        % Z_ALL_new((gg_1-1)*M_suba+1:gg_1*M_suba,:,iii) = kron(s_pilot_2(:,1).',W_g_set(:,:,1)');
        ...
        % Rx_NEW =pagemtimes(permute(H_set(:,:,AUD_Index_K_set),[1,3,2]),reshape(s_pilot_g_set(AUD_Index_K_set,:),[Ka,1,P_subc]));
        % Y_GMMV_new((gg_1-1)*M_suba+1:gg_1*M_suba,:,iii) = reshape(pagemtimes(permute(conj(W_g_set),[2,1,3]),Rx_NEW), [M_suba, P_subc]);

        % DDD = reshape(pagemtimes(permute(conj(W_g_set),[2,1,3]),BBB), [M_suba, P_subc]);
        % CCC = DDD *sqrt(Ptx);
        % AAA = reshape(pagemtimes(permute(conj(W_g_set),[2,1,3]),(Rx_Sig_Gmmv)), [M_suba, P_subc]);
        % sum(sum(abs(AAA-CCC)))

        for pp_2 = 1:P_subc
            Z_GMMV_ALL_set((gg_1-1)*M_suba+1:gg_1*M_suba,:,pp_2) = kron(s_pilot_g_set(:,pp_2).',W_g_set(:,:,pp_2)');
            % Z_GMMV_new((gg_1-1)*M_suba+1:gg_1*M_suba,:,pp_2,iii) = kron(s_pilot_g_set(:,pp_2).',W_g_set(:,:,pp_2)');
        end
    end
% end
% toc
% NNN = sum(sum(abs(Y_GMMV_new(:,:,4)-Y_GMMV_ALL_set(:,:,4))))
% NNN = sum(sum(abs(Y_GMMV_new(:,:,1)*sqrt(10^(Ptx_set(1)/10)*1e-3)-Y_GMMV_ALL_set(:,:,1))))
% NNN = max(max(max(abs(Z_GMMV_new(:,:,:,3)*sqrt(10^(Ptx_set(3)/10)*1e-3)-Z_GMMV_ALL_set(:,:,:,3)))))
% NNN = max(max(abs(Z_ALL_new(:,:,3)*sqrt(10^(Ptx_set(3)/10)*1e-3)-Z_ALL_set(:,:,3))))
%%
for ptx = 1:length(Ptx_set) % parfor or for
    Ptx_dBm = Ptx_set(ptx);   % Transmit power
    Ptx = 10^(Ptx_dBm/10)*1e-3;

    % Y_MMV_set=Y_ALL_set(:,:,ptx);
    % Z_MMV_set=Z_ALL_set(:,:,ptx);
    ...
    %     Y_GMMV_set((gg_1-1)*M_suba+1:gg_1*M_suba,:) = Y_GMMV_set((gg_1-1)*M_suba+1:gg_1*M_suba,:) + Y_GMMV_P2((gg_1-1)*M_suba+1:gg_1*M_suba,:);
    % end
    % toc
    ...
    [AUD_Index_set_BOMP, H_set_BOMP] = GMMV_StrBOMP(Y_GMMV_set, Z_GMMV_set, K, epsilon, M_suba);
    ...
    % [AUD_Index_set_OMPMMVsubA, H_set_OMPMMVsubA] = GMMV_OMP_subA_adv(Y_GMMV_set, Z_GMMV_set, K, epsilon, M_suba);
    ...
    fprintf('-------------------------------------\n');
    if Pe_BOMP_gg_nn > 0
        save debug.mat Y_GMMV_set Z_GMMV_set K epsilon M_suba Active_flag H_set
    end
    %% Display
    % if mod(nn_s,2) == 0
...
save GMMV_Ptx.mat Pe_BOMP_set NMSE_BOMP_set Pe_BOMP NMSE_BOMP_dB M_suba N_sub... 
    FacSp fs Ka awgn_en Ptx_set G_ofdm
...
h2 = legend('BSAMP','BSP','BOMP','MMV-BSOMP Adaptive','MMV-BSOMP','Location','southwest');
