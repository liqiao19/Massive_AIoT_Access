function [Nuser,Nsym,Nbit]=Perfor_AMP_sce1(lambda_new_sum,Amid2,H_est,K,Ka,N_seq,Na_seq,t_ind,T,bit,N_BS,GSMset,GSMindex,Data)
...
%     Nsym=(Ka-sum_act_user)*T+nnz( EstedSym-GSMindex(sub_matrix_correct,t_ind));
    Nsym=((Ka-sum_act_user+EU_FA)*T+nnz(EstedSym-GSMindex(sub_matrix_correct,t_ind)))/((Ka+EU_FA)*T);
    Nbit=0;
    for i=1:sum_act_user
