function [Nuser,Nsym,Nbit]=Perfor_AMP_angle(Pn,lambda_new_sum,Amid2,H_est,K,Ka,N_seq,Na_seq,T,bit,N_BS,GSMset,GSMindex,Data)
...
% Na_seq=opt.Na_seq;
%% Traditional AUD
NEW=reshape(lambda_new_sum,N_seq,K);
LAMBDA=sum(NEW,1);
[ACTsetThre2]=AUD_thresholdParfor(K,LAMBDA,0.9);

% H_ind=max(abs(H_est).');
% H_ind2=sum(reshape(H_ind.',N_seq,K),1);
% [ACTsetThre2]=AUD_thresholdParfor(K,H_ind2,40*Pn);

% Es_lamda_AMP2=H_ind2;
...
%         end
    ...
    Nsym=((Ka-sum_act_user+EU_FA)*T+nnz(EstedSym-GSMindex(sub_matrix_correct,:)))/((Ka+EU_FA)*T);
    Nbit=0;
    for i=1:sum_act_user
