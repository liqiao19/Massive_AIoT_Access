function [Nuser,Nsym,Nbit]=Perfor_AMP_tang_sce1(Index,Amid2,H_est,K,Ka,N_seq,Na_seq,t_ind,T,bit,N_BS,GSMset,GSMindex,Data)
...
% Na_seq=opt.Na_seq;
%% Traditional AUD
ADest=zeros(K*N_seq,1);
ADest(Index>0)=1;
TEMP=reshape(ADest,[N_seq,K]);
TEMP2=sum(TEMP);
Es_ABS_lamda=zeros(K,1);
Es_ABS_lamda(TEMP2~=0)=1;
...
%     Nsym=(Ka-sum_act_user)*T+nnz( EstedSym-GSMindex(sub_matrix_correct,t_ind));
Nsym=(nnz( EstedSym-GSMindex(sub_matrix_correct,t_ind))+(Ka-sum_act_user+EU_FA)*T)/((Ka+EU_FA)*T);
    Nbit=0;
    for i=1:sum_act_user
        EstedBit=de2bi(EstedSym(i,:)-1,bit,'left-msb');
        Nbit=Nbit+nnz(EstedBit-Data(t_ind,:,sub_matrix_correct(i)));
    end
%     Nbit=Nbit+(Ka-sum_act_user)*T*bit;
Nbit=(Nbit+(Ka-sum_act_user+EU_FA)*T*bit)/((Ka+EU_FA)*T*bit);
    % 没考虑多个时隙
    %% NMSE calculation
