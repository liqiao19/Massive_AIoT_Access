function [Nuser,Nsym,Nbit]=Perfor_AMP_Q_sce1(Pn,Y,S,L_Fr,lambda_new_sum,Amid2,H_est,K,Ka,N_seq,Na_seq,T,bit,N_BS,GSMset,GSMindex,Data)
...
%     Seff=S(:,INDEX);
%     He_lmmse=zeros(length(INDEX),size(Y,2));
% %     [He_lmmse] = OracleLMMSE(Y,Seff,Pn);
%     for i=1:size(Y,2)
%         He_lmmse(:,i)=0;
%     end
%     Hestest=abs(He_lmmse);
    ErrSymTemp=0;
    ErrBitTemp=0;
    for i=1:L_Fr
        for t=1:T
            absH_eq=ABS_H_est(INDEX,(i-1)*N_BS*T+(t-1)*N_BS+1:(i-1)*N_BS*T+t*N_BS);
%             absH_eq=Hestest(:,(i-1)*N_BS*T+(t-1)*N_BS+1:(i-1)*N_BS*T+t*N_BS);
        ...
        ErrSymTemp=ErrSymTemp+nnz( EstedSym-GSMindex(sub_matrix_correct,:));
        for kk=1:sum_act_user
            EstedBit=de2bi(EstedSym(kk,:)-1,bit,'left-msb');
            ErrBitTemp=ErrBitTemp+nnz(EstedBit-Data(:,:,sub_matrix_correct(kk)));
        end
    end
    Nsym=(ErrSymTemp+(Ka-sum_act_user+EU_FA)*T*L_Fr)/((Ka+EU_FA)*T*L_Fr);
%     Nsym=(Ka-sum_act_user)*T*L_Fr+ErrSymTemp;
    Nbit=(ErrBitTemp+(Ka-sum_act_user+EU_FA)*T*bit*L_Fr)/((Ka+EU_FA)*T*bit*L_Fr);
%     Nbit=ErrBitTemp+(Ka-sum_act_user)*T*bit*L_Fr;
    % 没考虑多个时隙
    %% NMSE calculation
